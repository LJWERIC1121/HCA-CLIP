...
def _test_preprocess(size):
    return [Resize(size, interpolation=BICUBIC), CenterCrop(size)]


def _basic_postprocess():
    funcs = [_convert_image_to_rgb, ToTensor(),
...
class Datum:
    ...
    Args:
        ...
        rotation (int): rotation angle id (0=0°, 1=90°, 2=180°, 3=270°), None if no rotation.
    """

    def __init__(self, impath='', label=0, domain=-1, classname='', rotation=None):
        assert isinstance(impath, str)
        assert isinstance(label, int)
        assert isinstance(domain, int)
        assert isinstance(classname, str)
        assert rotation is None or isinstance(rotation, int)
...
class DatasetWrapper(TorchDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Apply rotation if specified in item
        if hasattr(item, 'rotation') and item.rotation is not None:
            # rotation: 0=0°, 1=90°, 2=180°, 3=270°
            img0 = F.rotate(img0, angle=item.rotation * 90)
